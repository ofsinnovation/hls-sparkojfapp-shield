...
spark-submit --packages org.apache.spark:spark-streaming-kafka-0-8_2.11:2.0.1,TargetHolding/pyspark-cassandra:0.3.5     /home/centos/release1_5/ihealthdata/consumer/ihealth_anomaly_detection.py
'''
...
class Consumer(object):
    def __init__(self):
        ...
        self.data_access_layer = CassandraConnector()
    ...
    def activity_prediction_model_preparation(self, peopleid):
        ...
        pandas_df = self.data_access_layer.activity_historical_data_access(peopleid)
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  " + str(peopleid))

        spark_df = self.sqlContext.createDataFrame(pandas_df)
        df_dataset = spark_df.rdd.map(lambda row: LabeledPoint \
            (ACTIVITY_ENCODER_DICTIONARY[row.activityid], [row.heartrate, row.imuhandtemp]))
...
def last_element_parser(self,x):
    ...
    def Summary_Table_Updation(self, timestamp, heartbeat, people_id, seqno, activity_id):
        ...
        if (self.SUMMARY_TRANSITION_INDICATOR_FLAG_DICTIONARY[people_id]):
            ...
            if (self.ANOMALY_SCORE_MAX_DICTIONARY[people_id] > 0.8):
                '''
                self.data_access_layer.insert_cardiac_exception_summary_1(people_id, \
                                ...
                                self.ACTIVITY_PERSISTENCE_COUNTER_DICTIONARY[people_id])
                '''
                conn.insert_cardiac_exception_summary(people_id, \
                                                      self.START_TIME_NEW_SUMMARY_DICTIONARY[people_id], \
                                                      timestamp, \
                                                      seqno, \
                                                      activity_id, \
                ...
                conn.insert_activity_prediction_summary(people_id, \
                                                        self.START_TIME_NEW_SUMMARY_DICTIONARY[people_id], \
                                                        timestamp, \
                                                        seqno, \
                                                        activity_id, \
    ...
    def Numenta_Operations(self, current_time, people_id, actual_activity_id, \
        ...
        conn.insert_cardiac_exception(people_id, output_timestamp, \
                                      seqno, \
                                      activity_id, \
                                      isanomaly, \
                                      heartbeat, \
        ...
        if (isanomaly == "RED" and (people_id == 109 or people_id == 107)):
            anomaly_status = "RED"
            conn.people_anomaly_stat_insert(people_id, anomaly_status)
...
def data_parameter_extraction(self, pdf, current_stream_index, column_index, column_name, type_parser):
    ...
    def streamrdd_to_df(self, srdd):
        ...
        pdf_size = len(pdf.index)
                   # print(pdf_size)
        # Vinsent - Replace with Decorator / Lambda Map

            for current_stream_index in range(0, pdf_size):

            # Select the  cells with current_stream_index as row  in the column names from the pdf_string
            heartbeat = pdf_string.ix[current_stream_index,'heartrate']
            seqno = pdf_string.ix[current_stream_index,'seqno']
            activity_id = pdf_string.ix[current_stream_index,'activityid']
            timestamp = pdf_string.ix[current_stream_index,'datetime']
            people_id = pdf_string.ix[current_stream_index,'peopleid']
            imuhandtemp = pdf_string.ix[current_stream_index,'imuhandtemp']

            #	print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
            #	print( heartrate, seqno, activityid, timestamp, peopleid, imuhandtemp)
            # print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        ...
        ## Generating the Prediction Results
                    predictions = self.Activity_Prediction_Results(test_df,people_id)
        ...
        '''
        conn.insert_activity_prediction(people_id, current_time,   \
                seqno,activity_id, predicted_activity, self.PREDICTION_ACCURACY, heartbeat)
...
################################################################################################################
        ## Numenta Code
        self.Numenta_Operations(current_time, people_id, \
                                                  actual_activity_id, activity_id, timestamp, heartbeat, seqno)
...
def trigger_stream(self):
    self.kafkaStream = KafkaUtils.createDirectStream(self.ssc, ["test"], {"metadata.broker.list": hv.KAFKA_SCHEDULER})
