...
from ParserDecorator import ParserDecorator
import sys

sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject101')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject102')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject103')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject104')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject105')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject106')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject107')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject108')
sys.path.insert(0, '/home/centos/frontier-healthdata-services/ihealthdata/models/Numenta_Anomaly_Detect/Subject109')

from ihealthdata.models.Numenta_Anomaly_Detect.Subject101.Subject101_Run import model_Result_101, \
    create_Numenta_Model_101
from ihealthdata.models.Numenta_Anomaly_Detect.Subject102.Subject102_Run import model_Result_102, \
    create_Numenta_Model_102
from ihealthdata.models.Numenta_Anomaly_Detect.Subject103.Subject103_Run import model_Result_103, \
    create_Numenta_Model_103
from ihealthdata.models.Numenta_Anomaly_Detect.Subject104.Subject104_Run import model_Result_104, \
    create_Numenta_Model_104
from ihealthdata.models.Numenta_Anomaly_Detect.Subject105.Subject105_Run import model_Result_105, \
    create_Numenta_Model_105
from ihealthdata.models.Numenta_Anomaly_Detect.Subject106.Subject106_Run import model_Result_106, \
    create_Numenta_Model_106
from ihealthdata.models.Numenta_Anomaly_Detect.Subject107.Subject107_Run import model_Result_107, \
    create_Numenta_Model_107
from ihealthdata.models.Numenta_Anomaly_Detect.Subject108.Subject108_Run import model_Result_108, \
    create_Numenta_Model_108
from ihealthdata.models.Numenta_Anomaly_Detect.Subject109.Subject109_Run import model_Result_109, \
    create_Numenta_Model_109
...
class Consumer(object):
    def __init__(self):
        ...
        self.Row_Index = 0
        ####################################################################################
        self.CORRECT_PREDICTED_ACTIVITY_COUNTER = hv.CORRECT_PREDICTED_ACTIVITY_COUNTER
        self.PREDICTION_ACCURACY = hv.PREDICTION_ACCURACY

        self.START_TIME_NEW_SUMMARY_DICTIONARY = hv.START_TIME_NEW_SUMMARY_DICTIONARY_INITIALIZER
        ...
        ############################################################################
        self.ACTIVITY_PERSISTENCE_DICTIONARY = hv.ACTIVITY_PERSISTENCE_DICTIONARY

        self.MAX_HEARTRATE_DICTIONARY = hv.MAX_HEARTRATE_DICTIONARY

        self.MIN_HEARTRATE_DICTIONARY = hv.MIN_HEARTRATE_DICTIONARY

        self.SUMMARY_TRANSITION_INDICATOR_FLAG_DICTIONARY = hv.SUMMARY_TRANSITION_INDICATOR_FLAG_DICTIONARY

        self.ACTIVITY_PERSISTENCE_COUNTER_DICTIONARY = hv.ACTIVITY_PERSISTENCE_COUNTER_DICTIONARY

        self.SUM_HEARTRATE_DICTIONARY = hv.SUM_HEARTRATE_DICTIONARY

        self.SUM_RISK_SCORE_DICTIONARY = hv.SUM_RISK_SCORE_DICTIONARY

        self.ANOMALY_SCORE_MAX_DICTIONARY = hv.ANOMALY_SCORE_MAX_DICTIONARY

        self.PREDICTION_ACCURACY_SUB_INTERVAL_DICTIONARY = hv.PREDICTION_ACCURACY_SUB_INTERVAL_DICTIONARY

        self.SUM_CORRECT_PREDICTED_ACTIVITY_COUNTER_DICTIONARY = \
            hv.SUM_CORRECT_PREDICTED_ACTIVITY_COUNTER_DICTIONARY

        self.START_ACTIVITY_DICTIONARY = hv.START_ACTIVITY_DICTIONARY

        self.AVERAGE_HEARTRATE_DICTIONARY = hv.AVERAGE_HEARTRATE_DICTIONARY

        self.AVG_RISK_SCORE_DICTIONARY = hv.AVG_RISK_SCORE_DICTIONARY
    ...
    def spark_configurations(self):
        ...
        self.conf = (SparkConf() \
                     .setMaster(hv.MESOS_MASTER) \
                     .set("spark.executor.memory", hv.SPARK_EXECUTOR_MEMORY) \
                     .set("spark.cassandra.connection.host", hv.SPARK_CASSANDRA_CONNECTION_HOST))
        ...
        self.ssc = StreamingContext(self.sc, hv.SPARK_STREAMING_CONTEXT_DURATION)
    ...
    def activity_prediction_model_persistence_load(self):
        self.activity_prediction_model_101 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_101)
        self.activity_prediction_model_102 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_102)
        self.activity_prediction_model_103 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_103)
        self.activity_prediction_model_104 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_104)
        self.activity_prediction_model_105 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_105)
        self.activity_prediction_model_106 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_106)
        self.activity_prediction_model_107 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_107)
        self.activity_prediction_model_108 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_108)
        self.activity_prediction_model_109 = DecisionTreeModel.load(self.sc, hv.ACTIVITY_MODEL_PATH_109)
    ...
    def activity_prediction_model_preparation(self, peopleid):
        ...
        # (hv.ACTIVITY_ENCODER_DICTIONARY_INITIALIZER[row.activityid], [row.heartrate,row.imuhandtemp]))
        ...
        self.ACTIVITY_PREDICTION_MODEL_DICTIONARY[peopleid] = DecisionTree.trainClassifier( \
            trainingData, numClasses=hv.NUM_CLASSES, categoricalFeaturesInfo={}, \
            impurity=hv.IMPURITY, maxDepth=hv.MAX_DEPTH, maxBins=hv.MAX_BINS)
...
def last_element_parser(self,x):
    ...
    def activity_prediction_model_creation(self):
        ...
        map(self.activity_prediction_model_preparation, hv.SUBJECT_LIST_ID)
    ...
    def Summary_Table_Updation(self, timestamp, heartbeat, people_id, seqno, activity_id):
        ...
        if (self.SUMMARY_TRANSITION_INDICATOR_FLAG_DICTIONARY[people_id]):
            ...
            if (self.ANOMALY_SCORE_MAX_DICTIONARY[people_id] > 0.8):
                conn.insert_cardiac_exception_summary(int(people_id), \
                ...
                conn.insert_activity_prediction_summary(int(people_id), \
                                                        self.START_TIME_NEW_SUMMARY_DICTIONARY[people_id], \
                                                        timestamp, \
                                                        int(seqno), \
                                                        int(activity_id), \
    ...
    def Numenta_Operations(self, current_time, people_id, actual_activity_id, \
        ...
        conn.insert_cardiac_exception(int(people_id), output_timestamp, \
                                      int(seqno), \
                                      activity_id, \
                                      isanomaly, \
                                      heartbeat, \
        ...
        if (isanomaly == "RED" and (people_id == 109 or people_id == 107)):
            anomaly_status = "RED"
            conn.people_anomaly_stat_insert(int(people_id), anomaly_status)
        ...
        # print(duration,hv.SUMMARY_POLLING_DURATION)
        # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        ...
        if (duration > hv.SUMMARY_POLLING_DURATION or float(dictionary_activity_id) != activity_id):
            ...
            print
            int(self.SUM_HEARTRATE_DICTIONARY[people_id]), \
            self.ACTIVITY_PERSISTENCE_COUNTER_DICTIONARY[people_id]
...
def data_parameter_extraction(self, pdf, current_stream_index, column_index, column_name, type_parser):
    ...
    def streamrdd_to_df(self, srdd):
        ...
        # df Column rename
        pdf = deco.df_column_rename(df)
        #	print(pdf)
        pdf_string = pdf.astype(str)

        # print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")

        #	print(pdf_string.dtypes)

        # print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")

        pdf_string = pdf_string.applymap(deco.df_get_value)
        # print pdf_string
        ...
        for current_stream_index in range(0, pdf_size):
            ...
            '''
             # imuanklemagyaxis = deco.data_parameter_extraction \
            #		(pdf, current_stream_index, 0, 'imuanklemagyaxis', 'general_parser')
            # imuanklemagyaxis = int(imuanklemagyaxis.encode('ascii','ignore'))
            # print(imuanklemagyaxis)


            # seqno = deco.data_parameter_extraction \
                        #                  (pdf, current_stream_index, 1, 'seqno', 'general_parser')
            seqno = int(seqno.encode('ascii','ignore'))

             # activity_id = self.data_parameter_extraction \
                        #                 (pdf, current_stream_index, 2, 'activityId', 'general_parser')
            activity_id = str(int(activity_id.encode('ascii','ignore')))

                #	timestamp = self.data_parameter_extraction \
                        #                (pdf, current_stream_index, 3, 'time', 'time_parser')
            timestamp = timestamp.encode('ascii','ignore')

            # people_id = self.data_parameter_extraction \
                        #                 (pdf, current_stream_index, 4, 'peopleId', 'general_parser')
            people_id = int(people_id.encode('ascii','ignore'))

             #	imuhandtemp = self.data_parameter_extraction \
                        #                (pdf, current_stream_index, 5, 'imuhandtemp', 'last_element_parser')
            imuhandtemp = float(imuhandtemp.encode('ascii','ignore'))


                # print heartbeat, seqno, activity_id, timestamp, people_id, imuhandtemp
            '''
            ...
            print
            "Before Encoding activity_id = ", activity_id
            activity_id = self.enc.Encode_Label(activity_id)
            print
            "After Encoding activity_id = ", activity_id
            ...
            conn.insert_activity_prediction(int(peopleid), current_time, \
                                            int(seqno), activity_id, predicted_activity, self.PREDICTION_ACCURACY,
                                            heartrate)
    ...
    def trigger_stream(self):
        ...
        self.kafkaStream = KafkaUtils.createDirectStream(self.ssc, ["test"], \
                                                         {"metadata.broker.list": hv.KAFKA_SCHEDULER})
